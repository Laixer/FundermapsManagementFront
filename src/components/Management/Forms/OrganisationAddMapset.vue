...
const formHandler = async function (formData: { mapsetid: string }) {
  ...
  await addMapsetToOrganisation(props.record.id, formData.mapsetid)
}
</script>
...
<template>
  ...
  >
    <Input
      id="mapsetid"
      label="Add an existing mapset to the organisation"
      placeholder="Enter id of the mapset you want to connect"
      type="text"
